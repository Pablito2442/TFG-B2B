...
def run_analyze(settings: Settings) -> Path:
    ...
    with B2BGraphAnalyzer(
        ...
        risk_path = export_dict_to_json(
            analyzer.get_supplier_risk_concentration(top_n=10), export_dir, "risk_concentration.json"
        )
        discrepancy_sup_path = export_df_to_json(
            analyzer.get_discrepancy_rate_by_supplier(), export_dir, "discrepancy_by_supplier.json"
        )
        lead_time_path = export_df_to_json(
            analyzer.get_lead_time_compliance(), export_dir, "lead_time_compliance.json"
        )
        payment_path = export_df_to_json(
            analyzer.get_payment_terms_exposure(), export_dir, "payment_exposure.json"
        )
    ...
    payload = {
        ...
        "exported_files": [
            ...
            str(risk_path.name),
            str(discrepancy_sup_path.name),
            str(lead_time_path.name),
            str(payment_path.name),
